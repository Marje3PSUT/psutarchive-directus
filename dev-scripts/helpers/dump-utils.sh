...
dump_database_from_docker() {
    ...
    docker exec "$container_name" pg_dump --data-only --disable-triggers --column-inserts --no-comments --username "$DATABASE_USER" --file "/tmp/$dump_file" $table_params $db_name
