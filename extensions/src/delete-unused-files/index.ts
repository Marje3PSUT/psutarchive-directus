...
import { RESOURCES_FOLDER_ID } from "../constants";
...
export default defineHook(
  ({ schedule }, { services, database, getSchema, logger }) => {
    ...
    schedule("0 3 * * 0", async () => {
      ...
      const foldersItemsService = new ItemsService("directus_folders", {
        database: database,
        schema: schema,
      });
      ...
      const resFilesService = new FilesService({
        knex: database,
        schema: schema,
      });
